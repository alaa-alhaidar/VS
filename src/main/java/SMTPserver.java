...
public class SMTPserver {
    ...
    /* dictionary to store filename */
    private static Map<Integer, String> dictFile = null;
    ...
    public static void main(String[] args) throws IOException {
        ...
        dictFile = new HashMap<Integer, String>();
            ...
            while(iter.hasNext()) {
                ...
                if(key.isAcceptable()) {
                    ...
                    //received actual data ended in <CRLF>.<CRLF> -> save data to file
                        ...
                        if (s.length()>5) {
                            writeToFile(id, substringLeft(s, s.length()-5));//exclude <CRLF>.<CRLF>
                        }
                        dictFile.remove(id);
                        sendMessage("250 OK", buf, channel);
                    }
                    //write actual data line by line
                    else if (dictFile.containsKey(id)) {
                        if (s.equals(".\n")) {
                            dictFile.remove(id);
                            sendMessage("250 OK", buf, channel);
                        } else {
                    ...
                    //received DATA
                        ...
                        dictFile.put(id, getNewFilename(id));
    ...
    private static void writeToFile(Integer id, String data) {
        /* save string data to a file using JAVA NIO channel */
        String filename = dictFile.get(id);
        try {
            FileChannel ch = FileChannel.open(Paths.get(filename), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
    ...
    private static String getNewFilename (Integer id) {
